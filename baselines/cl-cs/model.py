...
def stratified_norm(out, n_samples):
    ...
    for i in range(n_subs):
        out_str[n_samples * i: n_samples * (i + 1), :] = (out[n_samples * i: n_samples * (i + 1), :] - out[n_samples * i: n_samples * (i + 1), :].mean(dim=0)) / (out[n_samples * i: n_samples * (i + 1), :].std(dim=0) + 1e-3)
    return out_str
...
class ConvNet_baseNonlinearHead(nn.Module):
    def __init__(self, n_spatialFilters, n_timeFilters, timeFilterLen, n_channs, stratified, multiFact, isMaxPool,
                 args):
        ...
        # # self.frequencyConv = nn.Conv2d(1249, 1249, (1, 1))
        # self.maxpool = nn.AvgPool2d((1, 30))
        ...
        # # self.frequencyConv2 = nn.Conv2d(64, 64, (1, 1))
    ...
    def forward(self, input):
        ...
        out = self.spatialConv(input)  # (30,16,1,1000)
        out = out.permute(0, 2, 1, 3)  # (30,1,16,1000)
        out = self.timeConv(out)  # (30,16,16,999)
        out = self.frequencyConv(out)  # (30,16,16,86)
        out = F.elu(out)  # 这里也有一个 那就这里画一个
        out = self.avgpool(out)  # (30,16,16,43)

        if 'middle1' in self.stratified:
            out = stratified_layerNorm(out, int(out.shape[0] / 2))

        out = F.elu(self.spatialConv2(out))  # (30,32,1,43)
        out = F.elu(self.timeConv2(out))  # (30,64,1,40)
        out = F.elu(self.frequencyConv2(out))  # (30,64,1,37)
        ...
        if self.isMaxPool:
            ...
            out = out.reshape(out.shape[0], -1)  # (30,1664)
            return out
