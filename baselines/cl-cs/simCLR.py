...
class SimCLR(object):
    ...
    def info_nce_loss(self, features, stratified):
        ...
        similarity_matrix = torch.corrcoef(features)
    ...
    def train(self, train_loader, val_loader, n_times, fold):
        # save config file
        ...
        for epoch_counter in range(self.args.epochs_pretrain):
            ...
            print(
                f"Epoch: {epoch_counter}   Train loss: {train_loss}   Top1 accuracy: {train_acc}   Top5 accuracy: {train_acc5}")
            print(
                f"\tVal loss: {val_loss}   Top1 accuracy: {val_acc}   Top5 accuracy: {val_acc5}")
            listloss.append((train_loss, val_loss))
            listacc.append((train_acc, val_acc, train_acc5, val_acc5))
        ...
        listloss = np.concatenate(listloss).reshape(-1, 2)
        listacc = np.concatenate(listacc).reshape(-1, 4)
        savepath = f'./recoder/{fold}'
        if not os.path.exists(savepath):
            os.makedirs(savepath)
        pd.DataFrame(listloss).to_csv(savepath + '/pre_loss.csv', index=False, header=False)
        pd.DataFrame(listacc).to_csv(savepath + '/pre_acc.csv', index=False, header=False)
        ...
        print(
            'best epoch: %d, train top1 acc:%.3f, top5 acc:%.3f; val top1 acc:%.3f, top5 acc:%.3f, train loss:%.4f, val loss: %.4f' % (
