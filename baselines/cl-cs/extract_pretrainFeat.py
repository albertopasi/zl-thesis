...
class ConvNet_baseNonlinearHead(nn.Module):
    ...
    def forward(self, input):
        ...
        # print('spatialConv', out.shape)
        out = out.permute(0, 2, 1, 3)
        out = self.timeConv(out)
        # print('timeConv', out1.shape)
        out = self.frequencyConv(out)
        out1 = out.clone()
        # print('frequencyConv', out.shape)
        out = F.elu(out)
        ...
        # print('spatialConv2', out.shape)
        out = F.elu(self.timeConv2(out))
        # print('timeConv2', out.shape)
        # out = F.elu(self.frequencyConv2(out))
        # print('frequencyConv2', out.shape)
        # print(out.shape)

        if 'middle2' in self.stratified:
            out = stratified_layerNorm(out, out.shape[0])
        # print('output', out.shape)
        # print('output1', out1.shape)
        return out, out1
...
parser.add_argument('--learning-rate', default=0.0007, type=float, metavar='LR', help='learning rate')
...
parser.add_argument('--use-data', default='pretrained', type=str, help='use which pretrained model')
...
for fold in range(n_folds):
    ...
    # print('features1_de1', features1_de.shape)
    print('fold', fold)
    ...
    if not randomInit:
        if args.dataset in ['first', 'second']:
            ...
            #     results_pretrain = pickle.load(f)
            with open(os.path.join(save_dir, 'folds_all_dataset_both_results_pretrain.pkl'), 'rb') as f:
                results_pretrain = pickle.load(f)
            best_pretrain_epoch = int(results_pretrain['best_epoch'][fold])
            checkpoint_name = 'checkpoint_{:04d}.pth.tar'.format(best_pretrain_epoch)
            print('load:', checkpoint_name)
    ...
    # print('features1_de2', features1_de.shape)
    n = 0
    for sub in val_sub:
        data_val = data[sub, :, :]
        ...
        # Prepare data
        # print('测试集', data_val.shape, label_val.shape, timeLen, timeStep, n_segs, fs)
        valset = EmotionDataset(data_val, label_val, timeLen, timeStep, n_segs, fs)
        val_loader = DataLoader(dataset=valset, batch_size=bn, pin_memory=True, shuffle=False)
