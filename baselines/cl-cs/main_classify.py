...
parser.add_argument('--max-tol', default=50, type=int, metavar='N', help='number of max tolerence for epochs with no val loss decrease in finetuning')
...
parser.add_argument('--epochs-pretrain', default=100, type=int, metavar='N', help='number of total epochs to run in pretraining')
...
parser.add_argument('--val-method', default='loo', type=str, help='10_folds or loo')
...
print('\nmain_classify.py\t--{}'.format(args.train_or_test))
...
timeStep = 1
filtLen = 1
sec = int((n_points / fs - timeLen) / timeStep + 1)
# sec = 30
# label_type = 'cls2'
# label_type = 'cls9'
    ...
    for base_long in [60]:
        ...
        for fold in range(n_folds):
            ...
            for iter in iteration:
                ...
                print('data_train', data_train.shape, label_train.shape)
                ...
                print('data_val', data_val.shape, label_val.shape)
                ...
                elif label_type == 'cls5':
                    model = simpleNN3(inp_dim, hidden_dim, 5, 30, stratified).to(args.device)
                elif label_type == 'cls3':
                ...
                if train_or_test == 'train':
                    ...
                    if len(best_epoch) > 2:
                        state_dict = os.path.join(save_dir_ft, str(fold),
                                                  'finetune_checkpoint_0%s.pth.tar' % (best_epoch))
                    else:
                        state_dict = os.path.join(save_dir_ft, str(fold),
        ...
        if train_or_test == 'test':
            ...
            sio.savemat('./result/Clisa_results_%s_%s.mat' % (label_type, val_method), mlp)
            pd.DataFrame(subjects_score).to_csv('./result/Clisa_score_%s_%s.csv' % (label_type, val_method))
            print('Save the val results.')

        # And this is test for the testset
