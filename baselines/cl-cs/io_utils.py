...
class EmotionDataset(Dataset):
    ...
    def __init__(self, data, label, timeLen, timeStep, n_segs, fs, transform=None):
        self.data = data.transpose()  # data转置（30，23310000）
        self.timeLen = timeLen  # 5
        self.timeStep = timeStep  # 2
        self.n_segs = n_segs  # 13
        self.fs = fs  # 250
        self.transform = transform
        self.label = torch.from_numpy(label)  # (40404,)
    ...
    def __getitem__(self, idx):
        ...
        n_samples_remain_each = 30 - self.n_segs * self.timeStep  # 4--5s的话（0，1，2，3，4）（2，3，4，5，6）就是0+4，2+4，就是4 这是啥 我感觉这里是步长
        # ((idx * self.timeStep + n_samples_remain_each * np.floor(idx / self.n_segs)) * self.fs)
        # 这一串比如idx=11655，这个11655是label对应的编号
        # 现在要根据label的编号去找data里面的数据，label=111*28*13
        # 他这个timeStep好像是要隔两秒画一个，还是化了之后隔两秒
        # 换算不来了，写得太好了 可以不管吗  想不出来 前面的我感觉是30s划分了13个片段后剩下的 ，30s都化了的
        # idx / self.n_segs--
        # n_samples_remain_each * np.floor(idx / self.n_segs)--
        one_seq = self.data[:,
                  int((idx * self.timeStep + n_samples_remain_each * np.floor(idx / self.n_segs)) * self.fs):
                  int((idx * self.timeStep + self.timeLen + n_samples_remain_each * np.floor(
                      idx / self.n_segs)) * self.fs)]
        # one_seq = self.data[:, int((idx * self.timeStep + (self.timeLen-1) * np.floor(idx / self.n_segs)) * self.fs):
        #                     int((idx * self.timeStep + self.timeLen + (self.timeLen-1) * np.floor(idx / self.n_segs)) * self.fs)]
        # print(idx, int((idx * self.timeStep + self.timeLen + (self.timeLen-1) * np.floor(idx / self.n_segs)) * self.fs))
        one_label = self.label[idx]
...
class TrainSampler():
    def __init__(self, n_subs, n_times, batch_size, n_samples):  # (111,1,28,(28,))
        self.n_per = int(np.sum(n_samples))  # 28*13=364
        self.n_subs = n_subs
