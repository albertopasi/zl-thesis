...
from src.approaches.lora_finetuning.config import LoRAConfig, OUTPUT_DIR
...
def print_fold_summary(
    ...
    print(
        ...
        f"|  LoRA {cfg.rank_tag}  |  {cfg.pool_tag}{cfg.cls_tag}{seed_label})"
    )
    print(f"{'=' * COL_W}")
    ...
    agg_path = (
        ...
        f"{cfg.cls_tag}_{cfg.rank_tag}{gen_tag_name}.json"
    )
...
def print_cross_seed_summary(
    ...
    print(
        ...
        f"|  LoRA {cfg.rank_tag}  |  {cfg.pool_tag}{cfg.cls_tag})"
    )
    print(f"{'=' * COL_W}")
    ...
    agg_path = (
        OUTPUT_DIR / f"summary_{cfg.task_mode}_{cfg.window_tag}_{cfg.pool_tag}"
        f"{cfg.cls_tag}_{cfg.rank_tag}_gen_cross_seed.json"
    )
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    agg = {
        "task_mode":    cfg.task_mode,
